...
setup_test_data <- function() {
  ...
  # Pin analyte by name so row-reordering in immunoplex_example$lod_lookup
  # cannot silently change which cytokine drives the fixture.
  target_cyto <- "Cytokine_01"
  stopifnot(
    target_cyto %in% lod_lookup$cytokine,
    !is.na(lod_lookup$lod[lod_lookup$cytokine == target_cyto])
  )
