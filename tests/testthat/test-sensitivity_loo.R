# Tests for leave_one_out_sensitivity()
#
# Validates:
...
describe("leave_one_out_sensitivity()", {
  ...
  test_that("returns tibble with expected columns", {
    ...
    result <- leave_one_out_sensitivity(
      data = dat,
  ...
  test_that("influential column uses threshold correctly", {
    ...
    result_low <- leave_one_out_sensitivity(
      data = dat,
      fixed = "timepoint + disease",
    ...
    result_high <- leave_one_out_sensitivity(
      data = dat,
      fixed = "timepoint + disease",
      target_terms = c("timepointT2"),
      random = "(1|subject_id)",
  ...
  test_that("errors on invalid input", {
    expect_error(
      leave_one_out_sensitivity("not_a_df", fixed = "x", target_terms = "y"),
  ...
  test_that("errors when subject_col is missing", {
    dat <- create_gaussian_test_data(n_subjects = 30, n_timepoints = 2)
    expect_error(
      leave_one_out_sensitivity(dat, fixed = "timepoint", target_terms = "timepointT2",
