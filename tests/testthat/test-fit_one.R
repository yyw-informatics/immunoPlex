...
test_that("random effects parameter validation protocol executes correctly", {
  ...
  # Subset below the configured subject threshold rather than assuming the
  # shared fixture is small. immunoplex_example carries 120 subjects, so the
  # random term is correctly retained on the full fixture; assuming otherwise is
  # what made this test fail when the example dataset was rebuilt.
  min_subjects <- get_immunoplex_config()$random_effects$min_subjects
  small_ids <- utils::head(unique(dat$subject_id), max(min_subjects - 1L, 2L))
  dat_small <- dat[dat$subject_id %in% small_ids, , drop = FALSE]
  expect_lt(length(unique(dat_small$subject_id)), min_subjects)

  # Should drop random effects below the subject threshold
  expect_message(
    fit_one(dat_small, family = "gamma", random = "(1|subject_id)"),
