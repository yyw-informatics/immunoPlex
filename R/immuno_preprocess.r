    ...
    if (aggregate_replicates) {
      df <- df %>%
        ...
        dplyr::summarise(
          ...
          lod           = dplyr::first(lod),
          # Preserve censoring flags through aggregation: a (subject, cytokine)
          # group is treated as censored if any of its replicates were
          # below LOD; the next branch reads `was_censored` to set `censored`.
          was_censored    = any(was_censored,    na.rm = TRUE),
          censored_below  = any(censored_below,  na.rm = TRUE),
          censored_above  = any(censored_above,  na.rm = TRUE),
