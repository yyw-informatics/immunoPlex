...
#'
#' @param expr_mat Matrix of cytokine expression (samples × cytokines)
...
#'   \item{expression}{matrix of preprocessed cytokine expression (samples × cytokines)}
              ...
              if (length(group1_vals) > 0 && length(group2_vals) > 0) {
                ...
                if (verbose) {
                  ...
                  if (p_val < balance_threshold) {
                    ...
                    cat(sprintf("    ✓ Balanced (p >= %.2f) - no adjustment\n", balance_threshold))
                  }
                }
              }
            } else {
