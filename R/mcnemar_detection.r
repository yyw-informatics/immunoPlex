...
# Used by method = "midp" and method = "noCC". Mirrors paired_wald_ci() in
# inst/simulations/benchmark_mcnemar.R for cross-comparison with B2.
...
#'   }
#'   Benchmark B2 (\code{inst/simulations/reports/B2_mcnemar_calibration.md}
#'   sections 5.1 and 5.8) finds that \code{"midp"} and \code{"noCC"} are
#'   better calibrated at small n while \code{"exact"} is conservative; we
#'   keep \code{"exact"} as the default since conservatism fails safely.
